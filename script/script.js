...
function myScript() {
  fetch(
    `https://frontend-mentor-apis-6efy.onrender.com/countries?search=${mainInput.value}`
  )
    .then((data) => data.json())
    .then((data) => {
      wrapper.innerHTML = "";
...
function myfilterScript() {
  fetch(
    `https://frontend-mentor-apis-6efy.onrender.com/countries?region=${filterRegion.value}`
  )
    .then((data) => data.json())
    .then((data) => {
      wrapper.innerHTML = "";
